...
// Example suggested IPC numbers - you can replace these with your actual suggestions
...
const Policecases = () => {
  ...
  const suggestedIPCs = ["IPC 302", "IPC 307", "IPC 376", "IPC 420"];

  const handleInputChange = (e) => {
    setIpcNumber(e.target.value);
  };

  const handleSuggestionClick = (ipc) => {
    setIpcNumber((prev) => (prev ? prev + ", " + ipc : ipc));
  };
  ...
  const [firEntries, setFirEntries] = useState([
    { firNumber: "", firDetails: "" },
  ]);
    ...
    const fetchCases = async () => {
      try {
        const response = await axios.post(
          "http://localhost:3001/getpolicases",
          { email }
        );
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-6">
                    <div className="bg-gray-50 rounded-xl p-4">
                      ...
                      <div className="space-y-3">
                        <DetailItem
                          icon={FileText}
                          label="Title"
                          value={selectedCase.complaintTitle}
                        />
                        <DetailItem
                          icon={AlertTriangle}
                          label="Type"
                          value={selectedCase.complaintType}
                        />
                        <DetailItem
                          icon={Calendar}
                          label="Date"
                        ...
                        <DetailItem
                          icon={Clock}
                          label="Time"
                          value={selectedCase.incidentTime}
                        />
                        <DetailItem
                          icon={MapPin}
                          label="Location"
                          value={selectedCase.incidentLocation}
                        />
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 rounded-xl p-4">
                      ...
                      <h3 className="text-lg mt-10 font-semibold text-gray-900 mb-4">
                        Docs Details
                      </h3>
                      <div className="bg-gray-50 rounded-xl p-4">
                        <h3 className="text-lg font-semibold text-gray-900 mb-4">
                        ...
                        {selectedCase.documents.map((victim, index) => (
                          <div
                            key={index}
                            className="mb-4 last:mb-0 border-b last:border-0 border-gray-200 pb-4 last:pb-0"
                          >
                            <div className="space-y-2">
                              <DetailItem
                                icon={FileText}
                                label="Document name"
                                value={victim.documentName}
                              />
                              <DetailItem
                                icon={Phone}
                                label="Doc CID NO"
                                value={
                                  <a
                                    href={`https://gateway.pinata.cloud/ipfs/${victim.cid}`}
                                    target="_blank"
                      ...
                      {selectedCase.fir.map((fir, index) => (
                        ...
                        >
                          <div className="space-y-2">
                            ...
                            <DetailItem
                              icon={Calendar}
                              label="Charges addition"
                              value={new Date(fir.firDate).toLocaleDateString()}
                            />
                            ...
                            <div className="mt-4">
                              ...
                              {fir.firEntries.map((entry, entryIndex) => (
                                <div key={entryIndex} className="space-y-2">
                                  <DetailItem
                                    icon={FileText}
                                    label="Entry IPC Number"
                                    value={entry.firNumber}
                                  />
                                  <DetailItem
                                    icon={FileText}
                                    label="Entry IPC Details"
                                    value={entry.firDetails}
                                  />
                                </div>
                              ))}
                            </div>
                      ...
                      <div key={index} className="space-y-4">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={document.cid}
                            onChange={(e) =>
                              handleCidChange(index, e.target.value)
                            }
                            className="w-full px-3 py-2 border text-black border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                      ...
                      <div key={index} className="space-y-4">
                        <div>
                          ...
                          <div className="mt-3">
                            <p className="text-sm text-gray-600 mb-2">
                              Suggested IPC Numbers:
                            </p>
                            <div className="flex flex-wrap gap-2">
                              {suggestedIPCs.map((ipc, index) => (
                                <button
                                  key={index}
                                  onClick={() => handleSuggestionClick(ipc)}
                                  className="bg-blue group flex items-center gap-1 px-3 py-1 bg-blue-100 border border-blue-300 rounded-full text-sm text-white hover:bg-blue-200 hover:border-blue-400 transition-colors duration-200"
                                >
                                  <span>{ipc}</span>
                                  <Plus className=" text-white w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity" />
                                </button>
                              ))}
                            </div>
                          </div>
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              handleFirDetailsChange(index, e.target.value)
                            }
                            className="w-full px-3 py-2 border text-black border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                            rows={3}
